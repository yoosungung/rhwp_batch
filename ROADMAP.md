# ROADMAP.md — rhwp-batch 구축 계획 (v0.6)
...
| MS | 제목 | 기간 | 주 산출물 |
|----|------|------|----------|
| M1 | **JSON 스키마 + HWP→JSON** | 1.5주 | DTO, 어댑터, 단위 테스트 |
| M2 | **CLI 골격 + `to-json` 명령** | 0.5주 | 바이너리 1차 빌드, 단일/디렉토리 변환, 5종 샘플 검증 |
| M3 | **양식 채우기 (`fill`, 텍스트)** | 1.5주 | 마커 치환, 단일 파일 채우기 |
| M4 | **배치 모드 + 표·이미지** | 1.5주 | `--data-dir`/`--manifest`, 행 확장, 이미지 |
| M5 | **컨테이너화 + 잡 통합** | 0.5주 | Dockerfile, K8s Job 템플릿, Airflow DAG 예시 |
| M6 | **HWPX 출력 (옵션)** | 1.5주 | `serialize_hwpx` 보완 또는 우회 |
...
## 15. 다음 행동 (작업지시자 승인 대기)

선결 사항 모두 완료. **M1 착수 승인** 시 다음 절차 진행:

1. GitHub Issue 등록 (예: `M1 #1 RAG-friendly DTO + HWP→JSON 변환기`)
2. `local/task{이슈번호}` 브랜치 생성
3. M1 수행 계획서 작성 → 작업지시자 승인 → 구현 계획서 → Stage 1 착수
4. 단계별 보고 → Stage 2 → ... → M1 최종 보고

> 결정 항목 D1~D29 모두 §1.1로 확정 완료. git 원격 `github.com/yoosungung/rhwp_batch`
> 셋업 완료(브랜치 `main`, commit 없음 = 초기 커밋이 첫 작업). 추가 결정 항목
> 발생 시 본 §15에 누적 기록.
